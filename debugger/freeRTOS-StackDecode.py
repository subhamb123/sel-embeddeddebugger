...
def readTaskStacks(filePath):
    ...
    with open(filePath, 'r') as file:
        for line in file:
            ...
            elif line.startswith("Task State:"):
                if currentTask is None:
                    raise ValueError("No task name found before task state data.")
                currentTaskState = line.split(":")[1].strip()
                tasks[currentTask]['task_state'] = currentTaskState
...
def main():
    ...
    for task, symbols in tasksWithSymbols.items():
        # Get the task's state as a numeric value
        task_state_numeric = int(tasksData[task].get('task_state', 0)) # Convert to integer
        # Look up the enumerated name based on the numeric state
        task_state_name = task_state_enum.get(task_state_numeric, 'Unknown')
        
        # Skip printing if task state is 0
        if task_state_numeric == 0:
            continue
        
        print(f"Task: {task}, State: {task_state_name}")
        print("Symbols:")
